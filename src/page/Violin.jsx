...
import { LoaderCircle, AlertTriangle, Music, Sliders, Waves } from 'lucide-react';
...
const PerformanceLineChart = ({ graphData, title, gradientId }) => {
    ...
    useEffect(() => {
        ...
        chart.append("path").datum(data).attr("fill", "none").attr("stroke", "#e879f9").attr("stroke-width", 2).attr("d", line);
...
// --- Main Violin Component ---
const Violin = ({ splitsId }) => {
    ...
    useEffect(() => {
        ...
        const fetchViolinData = async () => {
            ...
            setError(null);
            try {
                const response = await api.get(`/analytics/splits/violin/${splitsId}`);
                setData(response.data);
            } catch (err) {
                setError("Failed to load violin analysis.");
                console.error(err);
            } finally {
                setIsLoading(false);
            }
    ...
    if (isLoading) return <div className="flex items-center text-gray-400"><LoaderCircle size={20} className="animate-spin mr-2" />Loading Violin Details...</div>;
    if (error) return <div className="text-red-400 flex items-center"><AlertTriangle size={20} className="mr-2"/>{error}</div>;
    if (!data?.violin_description) {
        return <div className="text-gray-400">Data format from server is incorrect.</div>;
    }

    const { violin_description } = data;
    const { pitch, vibrato, performance_graph, articulation_and_timbre } = violin_description;
        ...
        <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div className="bg-gray-800/50 p-6 rounded-2xl border border-gray-700">
                    <h3 className="text-xl font-bold text-white mb-4 flex items-center"><Music size={22} className="mr-3 text-fuchsia-400"/>Pitch</h3>
                ...
                <div className="bg-gray-800/50 p-6 rounded-2xl border border-gray-700">
                    <h3 className="text-xl font-bold text-white mb-4 flex items-center"><Waves size={22} className="mr-3 text-fuchsia-400"/>Vibrato</h3>
                    <div className="space-y-2">
                ...
                <div className="bg-gray-800/50 p-6 rounded-2xl border border-gray-700">
                    <h3 className="text-xl font-bold text-white mb-4 flex items-center"><Sliders size={22} className="mr-3 text-fuchsia-400"/>Articulation & Timbre</h3>
                    <div className="space-y-2">
            ...
            <div className="w-full">
                <PerformanceLineChart 
                    ...
                    title={`Violin ${performance_graph.value_type} Over Time`}
                    gradientId="violin-gradient"
                />
            </div>
        </div>
    );
