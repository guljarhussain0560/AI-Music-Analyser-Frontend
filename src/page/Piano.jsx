...
import { LoaderCircle, AlertTriangle, TrendingUp, Music, BarChart, Zap, Play, Pause, Sun, Activity, Volume2, Layers } from 'lucide-react';
...
const PerformanceLineChart = ({ graphData, title, gradientId }) => {
    ...
    useEffect(() => {
        ...
        gradient.append("stop").attr("offset", "0%").attr("stop-color", "#8884d8").attr("stop-opacity", 0.4);
        gradient.append("stop").attr("offset", "100%").attr("stop-color", "#8884d8").attr("stop-opacity", 0);

        chart.append("path").datum(data).attr("fill", `url(#${gradientId})`).attr("d", area);
        chart.append("path").datum(data).attr("fill", "none").attr("stroke", "#8884d8").attr("stroke-width", 2).attr("d", line);
...
// --- Main PianoAnalysis Component ---
const Piano = ({ splitsId }) => {
    ...
    useEffect(() => {
        ...
        const fetchPianoData = async () => {
            setIsLoading(true);
            setError(null);
            try {
                // This component fetches data specifically for the 'piano' type
                const response = await api.get(`/analytics/splits/piano/${splitsId}`);
                setData(response.data);
            } catch (err) {
                setError("Failed to load piano analysis.");
                console.error(err);
            } finally {
                setIsLoading(false);
            }
    ...
    if (isLoading) return <div className="flex items-center text-gray-400"><LoaderCircle size={20} className="animate-spin mr-2" />Loading Piano Details...</div>;
    if (error) return <div className="text-red-400 flex items-center"><AlertTriangle size={20} className="mr-2" />{error}</div>;
    if (!data?.piano_description || !data?.piano_audio_url) return <div className="text-gray-400">Data format from server is incorrect.</div>;

    const { piano_description, piano_audio_url } = data;
    const { average_loudness, dynamic_variation, performance_graph, average_brightness, harmonic_complexity } = piano_description;
    ...
    return (
        <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <StatCard icon={<Activity size={24} />} label="Dynamic Variation" value={dynamic_variation?.toFixed(2)} />
                <StatCard icon={<Sun size={24} />} label="Average Brightness" value={average_brightness?.toFixed(2)} unit="Hz" />
                <StatCard icon={<Layers size={24} />} label="Harmonic Complexity" value={harmonic_complexity?.toFixed(2)} />
            </div>
            ...
            <div className="w-full">
                <PerformanceLineChart
                    ...
                    title={`Piano ${performance_graph.value_type} Over Time`}
                    gradientId="piano-gradient"
                />
            </div>

            {/* Audio Player Section */}
            <div className="w-full">
                <h3 className="text-2xl font-bold text-white mb-4 text-center">Isolated Piano Track</h3>
                <AudioPlayerWithVisualizer audioUrl={piano_audio_url} />
            </div>
